...
func TestUtils(t *testing.T) {
	...
	x1 := []any{"a", "b"}
	x2 := map[string]any{"a": "b"}
	x3 := make([]any, 0)
	x4 := make(map[string]any)
	if !hasValue(x1) {
		t.Errorf("%v", x1)
	}
	if !hasValue(x2) {
...
func TestMapUtils(t *testing.T) {
	map1 := map[string]any{"a": "x", "b": "y", "c": "z"}
	map2 := map[string]any{"a": "x", "b": "y", "c": "z"}
	identical, keyFound := compareMap(map1, map2)
	if !identical {
		t.Errorf("identical: %t, keyFound: %t", identical, keyFound)
	}

	map2 = map[string]any{"a": "x", "b": "y", "c": "zz"}
	identical, keyFound = compareMap(map1, map2)
	if identical || !keyFound {
		t.Errorf("identical: %t, keyFound: %t", identical, keyFound)
	}

	map2 = map[string]any{"a1": "x", "b1": "y", "c1": "z"}
	identical, keyFound = compareMap(map1, map2)
	if identical || keyFound {
		t.Errorf("identical: %t, keyFound: %t", identical, keyFound)
	}

	lists := []map[string]any{{"a": "x1", "b": "y", "c": "z"}, {"a": "x", "b": "y"}, {"a": "x3", "b": "y3", "c": "z3"}, {"a1": "x", "b1": "y", "c1": "z"}}
	...
	map2 = map[string]any{"a": "x1", "b": "y1", "c": "z1"}
	...
	map2 = map[string]any{"a1": "x1", "b1": "y1", "c1": "z1"}
	cmap12 = mergeMap(map1, map2)
	identical, keyFound = compareMap(cmap12, map[string]any{"a1": "x1", "b1": "y1", "c1": "z1", "a": "x", "b": "y", "c": "z"})
	if !identical {
		t.Errorf("identical: %t, keyFound: %t", identical, keyFound)
	}

	map2 = map[string]any{"a": "x1", "b1": "y1", "c1": "z1"}
	cmap12 = mergeMap(map1, map2)
	identical, keyFound = compareMap(cmap12, map[string]any{"a": "x1", "b1": "y1", "c1": "z1", "b": "y", "c": "z"})
	if !identical {
		t.Errorf("identical: %t, keyFound: %t", identical, keyFound)
	}

	map1 = map[string]any{"a": "x", "b": "y", "c": "z"}
	map2 = map[string]any{"a": "x", "b": "y", "c": "z1"}
	map12 := mergeMapOr(map1, map2)
	if len(map12.([]map[string]any)) != 2 {
		t.Errorf("%v", map12)
	}
	map2 = map[string]any{"a1": "x", "b": "y", "c": "z"}
	map12 = mergeMapOr(map1, map2)
	if len(map12.([]map[string]any)) != 2 {
		t.Errorf("%v", map12)
	}
	...
	if len(map12.(map[string]any)) != 3 {
		t.Errorf("%v", map12)
	}
	map2 = map[string]any{"a1": "x1", "b1": "y1", "c1": "z1"}
	map12 = mergeMap(map1, map2) // no common key
	identical, keyFound = compareMap(map12.(map[string]any), map[string]any{"a1": "x1", "b1": "y1", "c1": "z1", "a": "x", "b": "y", "c": "z"})
	if !identical {
		t.Errorf("identical: %t, keyFound: %t", identical, keyFound)
	}
}
