...
func TestContextProcedure(t *testing.T) {
	...
	dbi := &DBI{DB: db}
	dbi.logger = DevelopLogger(os.Stdout, slog.LevelDebug)
	...
	if string(lists[0].(map[string]any)["x"].(string)) != "m" {
		t.Errorf("%s m wanted", lists[0])
	}
	if string(lists[1].(map[string]any)["x"].(string)) != "n" {
	...
	lists = make([]any, 0)
	err = dbi.selectDoProcContext(ctx, &lists, hash, []any{[2]string{"y0", "int64"}}, sql, nil, "m")
	if err != nil {
